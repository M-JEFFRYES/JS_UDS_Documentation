...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|}
        ...
        & Accuracy: $\pm 1$ mL/s \\
        & Range: $0$--$50$ mL/s \\
        & Minimum flow recordable: $\leq 1$ mL/s \\
        \hline
        Volume void measurement 
        & Accuracy: greater of $\pm 3\%$ of true value or $\pm 2$ mL \\
        & Range: $0$--$1000$ mL \\
        \hline
...
\begin{itemize}
    \item Calibrated digital scales (precision $\pm 1$ g or better)
    \item Gravity-fed infusion tank (wider tanks preferred to maintain constant head pressure) with:
    \begin{itemize}
        \item Flexible pipe width (Pw) $\approx 6$–$7$ mm
        \item Adjustable flow clamp/valve (to vary flow rate)
    \end{itemize}
    \item Infusion fluid (density $\approx 1.00$ g/mL; water at room temperature is sufficient)
    \item Graduated cylinder or volumetric flask (1000 mL minimum)
...
\begin{itemize}
    ...
    \item Confirm the system reads 0 mL/s flow rate.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Re-zero (tare) the system so the displayed volume is 0 mL.
    \item Add known quantities of fluid (e.g., 100, 200, 400, 600, 800, 1000 mL) slowly into the container.
    \item Independently verify the true added volume using a calibrated digital scale  
          (1 g $\approx$ 1 mL for water-based fluid).
...
Mean difference $\leq \pm 3\%$ or $\pm 2$ mL.

\subsubsection*{(c) Flowrate Measurement Accuracy Test}

\paragraph{Purpose:}  
Verify that the flow rate calculations are accurate across the clinical range (0--50 mL/s).
...
\paragraph{Purpose:}  
Validate that the system can accurately record a flow rate of 1 mL/s.

\paragraph{Procedure:}
\begin{enumerate}
    \item Repeat the flowrate measurement procedure with a target of 1 mL/s.\\
    \textit{Note: This can be achieved using the gravity-fed infusion system with restricted outlet clamp.  \\
    Reference values:  
    $P_w = 1$ mm, $h_w = 169$ mm $\Rightarrow$ approx.\ 1 mL/s.}
\end{enumerate}
...
\paragraph{Procedure:}
\begin{enumerate}
    \item Start a continuous, slow fluid flow (e.g.\ $\leq 8$ mL/s \textit{– to ensure the max volume isn’t over 1000mL}).
